...
Public Class MyMain
    ...
    Public Function MainFunction(ByVal QuerySession As ISession) As IOQLObject
        ...
        Dim tTimecard As Query.Timecard = Query.Timecard.Current.MakeAlias(db)
        Dim tMatter As Query.Matter = Query.Matter.Current.MakeAlias(db)
        Dim tMatterAcl As New NxACLDenormalized("Matter")

        ' Main select
        Dim selectQ As ISelect = QueryBuilder.SelectFrom(tTimecard)
        Call selectQ.InnerJoin(tTimecard, tMatter, tTimecard.Matter.IsEqualTo(tMatter.MattIndex))
        ...
        Call caseStatement.When(tMatterAcl.HideConfidential.IsFalse(), tTimecard.Narrative)
        ...
        caseStatement = Exp.Case()
        Call caseStatement.When(tMatterAcl.HideConfidential.IsFalse(), tTimecard.Narrative.UnformattedText)
        Call caseStatement.Else(Exp.Null)
